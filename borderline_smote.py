...
class Borderlinesmote(ProcessedData):
    ...
    def findKNearest(self,  all_failure, single_failure):
        ...
        allfeature=np.array(self.feature_df)
        distance = np.sum(np.power((allfeature - single_failure), 2), axis=1)
        index = np.argsort(distance)[:2]
        n = 0
        label=np.array(self.label_df)
        for i in index:
            if label[i] == 0:
                n = n+1
            else:
                n = n-1
        if n > 0:
            dis = np.sum(np.power((all_failure - single_failure), 2), axis=1)
            aux_i = np.argsort(dis)[0]
